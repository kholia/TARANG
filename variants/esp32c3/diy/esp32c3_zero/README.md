...
This experimental build reports `has_default_channel=true` in MQTT MapReports for MeshMap compatibility. It does
not change TARANG's plaintext RF channel or zero PSK. MQTT downlink should remain disabled.

## Encrypted GPS tracker example

See [`examples/gps_tracker`](examples/gps_tracker/) for a minimal standalone GPS beacon that publishes
application-encrypted `PRIVATE_APP` packets over the open, relayable mesh through the Ra-02.
